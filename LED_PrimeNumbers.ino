...
void loop() {
  ...
  if (digitalRead(NOTIFY_PIN)) {  //LED pin went low and caused interrupt which set NOTIFY_PIN
    ...
    if ( ! initPass) {
      ...
      key = getIRKey();  //get key NOTE: waiting here<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      Serial.print("Key pressed: ");
      Serial.println(key);

      if (key == 186) { //info key - special demo mode

        //number line, every 10
        for (uint16_t ii = 0; ii < strip.numPixels(); ii++) {
          if ((ii % 10) == 0) {
            hsi2rgb( 0.0, 0.0, .99, rgb);
        ...
        key = getIRKey();
        delay(1000);
        //all green except 0,1
        for (uint16_t ii = 0; ii < strip.numPixels(); ii++) {
          hsi2rgb( 113, 0.99, .99, rgb);
          c = strip.Color(rgb[0], rgb[1], rgb[2]); //convert rgb value to 4 byte color value
          strip.setPixelColor(ii, c);
        ...
        while (key != 922) {
          ...
          //Set Divisor blue
          hsi2rgb( 170, 0.99, .99, rgb);
          c = strip.Color(rgb[0], rgb[1], rgb[2]); //convert rgb value to 4 byte color value
          strip.setPixelColor(divisor, c);
          strip.show();
          key = getIRKey();
          delay(1000);
          //set numbers divisible by divisor and still on red
          offcount = 0;
          hsi2rgb( 0.0, .99, .99, rgb);  //red
          c = strip.Color(rgb[0], rgb[1], rgb[2]);
          for (uint16_t ii = divisor + 1; ii < strip.numPixels(); ii++) {
            if ((ii % divisor) == 0 && strip.getPixelColor(ii) != 0 ) {
              offcount++;
              strip.setPixelColor(ii, c);
            }
          }
          ...
          key = getIRKey();
          delay(1000);
          //turn off divisible by divisor if there are any
          if (offcount > 0) {
            for (uint16_t ii = divisor + 1; ii < strip.numPixels(); ii++) {
              if ((ii % divisor) == 0) {
                if (strip.getPixelColor(ii) != 0) {
                  strip.setPixelColor(ii, 0);
                }
              }
            }
            strip.show();
            key = getIRKey();  //get key NOTE: waiting here in special demo mode
            delay(1000);
          }
          divisor++;
        } //end key != 922
      }  //end info 186 (special mode)
    } else {  // end not init pass
      OpMode = OpMode_Play;
      initPass = false;
      Serial.println("Initial Pass - Starting PLAY mode");
    }
    switch (key) {
      case 128: //l(one) key no lights on
  ...
  while (!digitalRead(NOTIFY_PIN)) { //do this forever
    ...
    for ( j = 0; j < strip.numPixels() - 4; j++) {
      ...
      for ( i = j + 4; i < strip.numPixels(); i++) {
        strip.setPixelColor(i, 0); //turn remaining pixels off
        if (digitalRead(NOTIFY_PIN)) {
          Serial.println("Pack() returning due to NOTIFY_PIN");
          return;
        }
